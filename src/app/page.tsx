...
const [selectedCol, setSelectedCol] = useState<number>(0);
  ...
  const initBoard = useCallback(() => {
    ...
    const offsetRow = Math.max(0, Math.min(config.rows - 1, centerRow + Math.floor(Math.random() * 5) - 2));
    const offsetCol = Math.max(0, Math.min(config.cols - 1, centerCol + Math.floor(Math.random() * 5) - 2));
    ...
    setTimeout(() => {
      ...
      for (let radius = 0; radius < Math.max(config.rows, config.cols) && !found; radius++) {
        for (let r = Math.max(0, offsetRow - radius); r <= Math.min(config.rows - 1, offsetRow + radius); r++) {
          for (let c = Math.max(0, offsetCol - radius); c <= Math.min(config.cols - 1, offsetCol + radius); c++) {
  ...
  const resetBoardForDifficulty = (newDifficulty: Difficulty): void => {
    ...
    const offsetRow = Math.max(0, Math.min(cfg.rows - 1, centerRow + Math.floor(Math.random() * 5) - 2));
    const offsetCol = Math.max(0, Math.min(cfg.cols - 1, centerCol + Math.floor(Math.random() * 5) - 2));
    setSelectedRow(offsetRow);
    setSelectedCol(offsetCol);
  };
  ...
  useEffect(() => {
    const updateCellSize = () => {
      ...
      const containerWidth = gameCardRef.current?.clientWidth ?? window.innerWidth;
      const available = Math.max(100, containerWidth - paddingTotal - gapTotal - 40); // leave some room for UI
  ...
  const placeMines = (excludeRow: number, excludeCol: number): number[][] => {
    ...
    while (minesPlaced < config.mines) {
      ...
      const isInExcludeNeighborhood = Math.abs(row - excludeRow) <= 1 && Math.abs(col - excludeCol) <= 1;
  ...
  const buildBoardFromMines = (mines: Array<[number, number]>): number[][] => {
    const newBoard: number[][] = [];
    for (let i = 0; i < config.rows; i++) {
      newBoard[i] = [];
      for (let j = 0; j < config.cols; j++) {
  ...
  const revealCellRecursive = (
    ...
    if (!isValid(row, col) || currentRevealed[row][col] || flagged[row][col]) {
      return;
    }
    ...
    if (currentBoard[row][col] === 0) {
      for (let i = -1; i <= 1; i++) {
        for (let j = -1; j <= 1; j++) {
          revealCellRecursive(row + i, col + j, currentBoard, currentRevealed);
        }
      }
    }
  };
  ...
  const endGame = (won: boolean, currentBoard: number[][]): void => {
    setGameOver(true);
    setGameWon(won);
    setIsRunning(false);
  ...
  }, [selectedRow, selectedCol, board, config.rows, config.cols, gameOver, revealed, flagged]);
    ...
    <>
      ...
      <div className="game-container">
        <div className="game-card" ref={gameCardRef}>
          ...
          {gameOver ? (
            <div className={`text-center mb-3 h3 fw-bold ${gameWon ? 'text-success' : 'text-danger'}`}>
              ...
              {board.map((row, i) =>
                row.map((_, j) => (
                  <div
                    ...
                    onClick={() => { setSelectedRow(i); setSelectedCol(j); handleCellClick(i, j); }}
