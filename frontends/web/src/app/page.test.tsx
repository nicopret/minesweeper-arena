...
describe("Minesweeper page component", () => {
  ...
  it("renders instructions and controls after mount", async () => {
    ...
    expect(screen.getByTestId("flags-display")).toBeInTheDocument();
  });
  ...
  it("updates flag counter when flagging a cell", async () => {
    ...
    const initialFlagDisplay = await screen.findByTestId("flags-display");
  ...
  it("keeps playing after flagging some mines (no premature win)", async () => {
    ...
    // Reveal a safe cell to trigger win detection after flags placed
    await act(async () => {
  ...
  it("updates mine counter when changing difficulty", async () => {
    ...
    const counter = await screen.findByTestId("flags-display");
    const initial = parseInt(
      counter.textContent?.replace(/\D/g, "") || "0",
      10,
    );

    const hardBtn = screen.getByRole("button", { name: /Hard \(5x5\)/i });
    await act(async () => {
